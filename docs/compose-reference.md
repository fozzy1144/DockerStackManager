...
- **Complete services**
  - [Minimal web service](#minimal-web-service)
  - [PostgreSQL with a named volume](#postgresql-with-a-named-volume)
  - [App that waits for its database](#app-that-waits-for-its-database)
  - [Scheduled job (run and exit)](#scheduled-job-run-and-exit)
- **Networking & ports**
  - [Port publishing variants](#port-publishing-variants)
  - [Custom network with aliases](#custom-network-with-aliases)
  - [Join an existing external network](#join-an-existing-external-network)
  - [Host networking](#host-networking)
- **Storage**
  - [Named volume](#named-volume)
  - [Bind mounts and tmpfs](#bind-mounts-and-tmpfs)
  - [NFS volume](#nfs-volume)
- **Health & lifecycle**
  - [Healthcheck variants](#healthcheck-variants)
...
- **Environment & secrets**
  - [Environment and .env interpolation](#environment-and-env-interpolation)
- **Logging**
  - [Log rotation](#log-rotation)
- **Reverse proxy**
  - [Traefik labels](#traefik-labels)
- **Reuse & structure**
  - [YAML anchors for shared settings](#yaml-anchors-for-shared-settings)
...
*The smallest service worth deploying: pinned, restarting, health-checked.*
...
Change `8080` to the host port you want; the `80` after the colon is
...
*Database with durable storage, a real healthcheck, and no published port.*
...
Note what is *missing*: there is no `ports:` block. A database only
...
### App that waits for its database

*Ordered startup using depends_on with a health condition.*
...
`condition: service_healthy` is the difference between "started" and
...
### Scheduled job (run and exit)

*One-shot container for backups or maintenance, not restarted.*
...
`profiles: ["tools"]` keeps this out of a normal `up -d`. Run it
...
Reference: <https://docs.docker.com/reference/compose-file/services/#profiles>

---

## Networking & ports

### Port publishing variants

*Every form of the ports mapping, and what each one exposes.*
...
The rule for the short form is `[host_ip:][host_port:]container_port`.
...
*Separate front-end and back-end networks so the database is unreachable from outside.*
...
Then give each service only the networks it needs:
...
Reference: <https://docs.docker.com/reference/compose-file/networks/>

### Join an existing external network

*Attach to a proxy network another stack owns.*
...
`external: true` tells Compose the network already exists and must not
...
Reference: <https://docs.docker.com/reference/compose-file/networks/#external>

### Host networking

*Share the host's network stack — for mDNS, DHCP, or VPN containers.*
...
In host mode the container has no network namespace of its own: it
...
Reference: <https://docs.docker.com/reference/compose-file/services/#network_mode>
...
*Docker-managed storage that survives recreation.*
...
A named volume with no options is the normal case, and the empty value
...
Reference: <https://docs.docker.com/reference/compose-file/volumes/>

### Bind mounts and tmpfs

*Mount host paths and RAM-backed scratch space.*
...
Relative paths resolve against the directory holding the compose file,
...
Set `addr` to the NAS address and `device` to the exported path, with
the leading colon.
...
Reference: <https://docs.docker.com/reference/compose-file/volumes/#driver_opts>
...
*HTTP, TCP, and CLI health probes with the timing that matters.*
...
`CMD` runs the command directly; `CMD-SHELL` runs it through `sh -c`,
...
*The four policies, and which one you actually want.*
...
`unless-stopped` is the right default for a long-running service. It
...
Reference: <https://docs.docker.com/reference/compose-file/services/#restart>

### Graceful shutdown

*Give a service time to finish, and reap zombie processes.*
...
`stop_grace_period` is how long Docker waits after the stop signal
...
Reference: <https://docs.docker.com/reference/compose-file/services/#stop_grace_period>
...
*Stop one container from taking down the host.*
...
`limits` is a hard ceiling; `reservations` is a soft guarantee used for
...
Reference: <https://docs.docker.com/reference/compose-file/deploy/#resources>
...
*Drop privileges, drop capabilities, read-only root filesystem.*
...
Apply these one at a time and test between each; several will break a
...
Reference: <https://docs.docker.com/reference/compose-file/services/#cap_drop>

### File-based secrets

*Pass credentials as files instead of environment variables.*
...
Reference: <https://docs.docker.com/reference/compose-file/secrets/>

---

## Environment & secrets

### Environment and .env interpolation

*Both environment syntaxes, plus defaults and required variables.*
...
There are two distinct mechanisms and confusing them causes real
trouble:
...
By default Docker's json-file logs grow without limit. A chatty
...
*Expose a service through Traefik with automatic TLS.*
...
Replace `myapp` throughout with a unique name per router — reusing one
...
### YAML anchors for shared settings

*Define common service settings once and merge them everywhere.*
...
Use it in each service with the merge key `<<`:
...
Reference: <https://docs.docker.com/reference/compose-file/extension/>

### Override files

*Layer environment-specific changes without editing the base file.*

Placement: **root**

```yaml
# docker-compose.override.yml — applied automatically on top of the base
services:
  app:
    environment:
      DEBUG: "true"
...
`docker compose up` reads `docker-compose.yml` and, if present,
...
Reference: <https://docs.docker.com/reference/compose-file/merge/>

### Watchtower auto-updates

*Automatic image updates, with the caveats spelled out.*
...
Opt services in individually with a label, which is what
...
| error | Named volume or network used but not declared | `up` fails; a missing `./` turns a bind mount into an undeclared volume. |
| error | `depends_on` naming an unknown service | Refers to service keys, not container names. |
| error | Two services publishing the same host port | The second container will not start. |
| error | Duplicate `container_name` | Container names are host-wide. |
| warning | `latest` or untagged image | The next pull can cross a major version. |
| warning | No restart policy | The service will not return after a reboot. |
| warning | `privileged: true` | Disables container isolation. |
| warning | Literal password/token in `environment` | Visible to anyone who reads the file or runs `docker inspect`. |
| info | Obsolete `version:` key | Ignored by Compose v2. |
| info | No healthcheck | `depends_on: service_healthy` cannot wait, and the app cannot report a stack as partially up. |
